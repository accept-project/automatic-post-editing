...
def teralign(inputWords, refWords):
  """Compute the alignment that minimises the translation
     edit rate"""
  score,shifts = ter(inputWords, refWords)
  shifted_input = inputWords
  input_pos = range(len(inputWords))
  for shift in shifts:
    input_pos = _apply_shift(input_pos,*shift)
    shifted_input = _apply_shift(shifted_input,*shift)
  edits,aligns = edit_distance(shifted_input,refWords)
  aligns = {src:tgt for (src,tgt) in aligns}
  composed_aligns = []
  for src,tgt in enumerate(input_pos):
    if aligns.has_key(src):
      composed_aligns.append((tgt,aligns[src]))
  return composed_aligns
...
def _ter(iwords, rwords, mtd):
    ...
    shifts = []
    while True:
        delta, new_iwords,shift = _shift(iwords, rwords, mtd)
        # print('[I]', u' '.join(iwords))
        # print('[R]', u' '.join(rwords))
        # print('[ED]', mtd(iwords))
        if delta <= 0:
        ...
        shifts.append(shift)
        iwords = new_iwords
    return (err + mtd(iwords)) / len(rwords), shifts

def _apply_shift(iwords, isp, rsp, length):
...
def _shift(iwords, rwords, mtd):
    ...
    for isp, rsp, length in _findpairs(iwords, rwords):
        shifted_words = _apply_shift(iwords,isp,rsp,length)
        scores.append((pre_score - mtd(shifted_words),shifted_words , (isp, rsp, length)))
    if not scores:
        return (0, iwords, None)
    scores.sort()
    return scores[-1]
...
def edit_distance(s, t):
    ...
    bp = _gen_matrix(len(s) + 1, len(t) + 1, None) # back pointers
    l[0] = [x for x, _ in enumerate(l[0])]
    for x, y in enumerate(l):
        y[0] = x
    for i, j in itrt.product(range(1, len(s) + 1), range(1, len(t) + 1)):
        prevs = [((i-1,j), l[i - 1][j] + 1), \
                 ((i, j-1), l[i][j - 1] + 1), \
                 ((i-1,j-1), l[i - 1][j - 1] + (0 if s[i - 1] == t[j - 1] else 1))] 
        prevs.sort(key=lambda x: x[1])
        l[i][j] = prevs[0][1]
        bp[i][j] = prevs[0][0]

    # chase back-pointers to get the alignment
    align = []
    cur = (len(s), len(t))
    while bp[cur[0]][cur[1]]:
      prev = bp[cur[0]][cur[1]]
      if prev[0] != None:
        if prev[0]+1 == cur[0] and prev[1]+1 == cur[1]:
          align.append((cur[0]-1,cur[1]-1))
      cur = prev
    align.reverse()
    #return l[-1][-1],aligns,alignt,align
    return l[-1][-1],align
