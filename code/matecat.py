...
class Segment(object):
    def __init__(self,DB,fid,S):
        ...
        c.execute("""SELECT suggestion FROM segment_translations where id_segment = %s""",(self.id))
        self.sugg = [t for t in c]
        return
    
    def xliff_str(self):
        ret  = '<trans-unit id="%d">\n'%int(self.seqno)
        ret += '<source>%s</source>\n'%self.text
        for t in self.trans: ret += '<target>%s</target>\n'%t['translation']
        for t in self.sugg: ret += '<mt>%s</mt>\n'%t['suggestion']
        ret += '</trans-unit>\n'
        return ret
...
class File(object):
    ...
    def xliff_str(self,l1,l2,dt,fname=None):
        ret  = '<file source-language="%s"'%l2
        ret += ' target-language="%s"'%l2
        if fname:
            ret += ' original="%s" datatype="%s">\n<body>\n'%(fname,dt)
        else:
            ret += ' original="%s" datatype="%s">\n<body>\n'%(self.id,dt)
            pass
        for s in self.segments: ret += s.xliff_str()
        ret += '</body>\n</file>\n'
        return ret
...
class Job(object):
    ...
    def xliff_str(self,fname):
        F = [f.xliff_str(self.source,self.target,'plaintext',fname) 
             for f in self.files]
        return '\n'.join(F)
